...
async function testExceededPluginLimit(api: types.IExtensionApi, infoCache: PluginInfoCache) {
...
async function testMissingMasters(api: types.IExtensionApi, infoCache: PluginInfoCache) {
  const { translate, store } = api;
  const state = store.getState();
  const gameMode = selectors.activeGameId(state);
  if (!gameSupported(gameMode)) {
 ...
 */
async function testBlueprintMasters(api: types.IExtensionApi, infoCache: PluginInfoCache) {
  const { translate, store } = api;
  const state = store.getState();
  const gameMode = selectors.activeGameId(state);
