...
- **Upstream `.github/workflows/fingerprint-*.yml` use `runs-on: [self-hosted]` targeting a Nexus-Mods-internal runner.** Forks have no such runner, so every tag push queues a `Fingerprints - Mark as released` job that sits pending until GitHub's 72-hour timeout kills it. Same story for `Fingerprints - Mark as fixed` on every merged PR and `Fingerprints - Manually resolve` on `workflow_dispatch`. Harmless (no effect on Release Linux or user-visible builds), but noisy in the Actions tab. Fix: gate each `sync` job with `if: github.repository == 'Nexus-Mods/Vortex'` so forks no-op cleanly while upstream behaviour stays untouched. If an upstream rewrite drops the gate, pending runs return immediately — grep the three workflow files after any merge that touches `.github/workflows/`. Commit `7b155e5e1`.
...
| Gate `fingerprint-*.yml` workflows to `Nexus-Mods/Vortex` repo only                | `7b155e5e1` | _master-only_ |
