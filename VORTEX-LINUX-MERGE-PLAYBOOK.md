...
- **Upstream `.github/workflows/fingerprint-*.yml` use `runs-on: [self-hosted]` targeting a Nexus-Mods-internal runner.** Forks have no such runner, so every tag push queues a `Fingerprints - Mark as released` job that sits pending until GitHub's 72-hour timeout kills it. Same story for `Fingerprints - Mark as fixed` on every merged PR and `Fingerprints - Manually resolve` on `workflow_dispatch`. Harmless (no effect on Release Linux or user-visible builds), but noisy in the Actions tab. Fix needs TWO parts: (1) gate each `sync` job with `if: github.repository == 'Nexus-Mods/Vortex'` so forks no-op logically; (2) make `runs-on` conditional — `${{ github.repository == 'Nexus-Mods/Vortex' && 'self-hosted' || 'ubuntu-latest' }}` — because GitHub evaluates `runs-on` and queues the workflow waiting for a runner BEFORE it evaluates the job-level `if`. Without the conditional `runs-on`, the workflow still sits pending waiting for a self-hosted runner that doesn't exist in the fork, even though the job would have skipped. Part (1) alone (commit `7fd37ff71`) was incomplete — confirmed by a pending `Fingerprints - Mark as released` run on the v2.0.0-linux re-tag. Part (2) lands in a follow-up commit. If an upstream rewrite drops either piece, pending runs return immediately — grep the three workflow files for `self-hosted` (should only appear inside the ternary) after any merge that touches `.github/workflows/`.
