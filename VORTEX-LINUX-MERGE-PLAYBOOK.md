...
## §13 action bundle dirs ignored by oxfmt

```bash
grep -n '"\.github/actions' .oxfmtrc.json
```

`.oxfmtrc.json` `ignorePatterns` must include a pattern covering every `.github/actions/*/dist/**` bundle. Originally added as the literal `.github/actions/fingerprints/dist/**` (`fe07ccee6`, after v2.0.2 sync surfaced the breakage on PR #6), then generalised to the glob `.github/actions/*/dist/**` sometime before the v2.6.3 sync — the glob subsumes the original literal and additionally covers every new upstream composite action with a bundled `dist/` (e.g. `linear-release`, added in v2.6.x). **No smoke probe covers this item** — `scripts/linux-smoke.sh` has zero references to it; it's a format-gate concern, not a Linux-behavior one, so verify it with `pnpm run format:check` after a merge rather than expecting the smoke script to catch a regression here. Without the ignore, every routine sync trips oxfmt's `format:check` on generated bundle lines we don't own and aren't going to format-check our way around.
...
### pnpm-workspace.yaml catalog entries silently dropped for fork-only devDeps

- **A 3-way merge can silently delete a `pnpm-workspace.yaml` catalog line with zero conflict, if our side never touched it and upstream deleted it.** v2.6.3 sync: upstream removed the `"@types/i18next": 12.1.0` catalog entry (modern `i18next` ships its own types, so the stub package became unnecessary upstream) while our fork-only `extensions/gamebryo-ba2-support/package.json` still referenced `"@types/i18next": "catalog:"` — that extension doesn't exist upstream, so upstream had no reason to know the entry was still load-bearing on our side. Standard 3-way merge semantics ("unchanged on our side + deleted on theirs = accept the deletion") applied cleanly, no `UU` conflict, `pnpm-workspace.yaml` auto-merged — and `pnpm install` then failed hard with `[ERR_PNPM_CATALOG_ENTRY_NOT_FOUND_FOR_SPEC] No catalog entry '@types/i18next' was found for catalog 'default'`.
- **Fix: restore the specific catalog line from the pre-merge tree, don't touch the extension's package.json.** `git show <merge-base>:pnpm-workspace.yaml | grep '"@types/<pkg>"'` to recover the exact version pin, re-insert it at its alphabetical position.
- **Rule: any `pnpm install` failure with `ERR_PNPM_CATALOG_ENTRY_NOT_FOUND_FOR_SPEC` after a sync is this class** — grep every fork-only extension's `package.json` for `"catalog:"` references and cross-check each one still has a matching entry in `pnpm-workspace.yaml`'s `catalog:` block; a clean auto-merge on the workspace file proves nothing, since this exact deletion shape never raises a conflict. v2.6.3 sync, discovered live rather than via a pre-existing playbook check.
...
- **`resolvePathCase`'s "no on-disk match" branch keeps source casing — wrong for Bethesda games on ext4. (DIAGNOSED 2026-05-22, NOT YET FIXED.)** `resolvePathCase` is asymmetric by design: when a directory segment matches an existing on-disk entry (case-insensitive), it adopts the disk casing; when no match exists, it preserves the source segment so `ensureDir` can create it later (see [resolvePathCase.ts:65](src/renderer/src/util/resolvePathCase.ts#L65) and [LinkingDeployment.ts:799](src/renderer/src/extensions/mod_management/LinkingDeployment.ts#L799)). On Windows that asymmetry is invisible — case-insensitive fs. On ext4 it's the deployment-casing bug: first-time deploy of `Behaviors/0_Master.hkx` from a Pandora/Nemesis/FNIS-style archive creates `Data/Behaviors/0_Master.hkx`, but the Skyrim/Bethesda engine string-matches `behaviors\0_master.hkx` (lowercase by Bethesda convention — BSA paths are all lowercase). Result: behaviour graph never loads, NPCs freeze, attacks don't register. Looks like a broken save / leftover mod; it's a deploy-layer bug. **Suggested shape:** lowercase output path components when `gameId` belongs to a Bethesda title (Skyrim LE/SE/AE/VR, Fallout 3/NV/4/76, Oblivion, Morrowind, Starfield) and the target fs is case-sensitive. Narrow-by-game is safer than broad-by-platform — non-Bethesda games may legitimately ship case-sensitive content (e.g. shader-pack mods that NIF-reference textures verbatim). Probe target case-sensitivity at deploy start (write `foo` + `FOO`, see if both resolve) and gate normalisation on that. The fix surface is `resolvePathCase` (add an `engineLowercase: bool` parameter) plus [LinkingDeployment.ts:793](src/renderer/src/extensions/mod_management/LinkingDeployment.ts#L793) (pass it). Whether to also lowercase the manifest's `relPath` is a follow-on call — current manifest stores source casing verbatim and `externalChanges` already case-resolves at lookup, so manifest can stay verbatim if `resolvePathCase` is the single normalisation point.
...
### tool outputs & orphan cleanup

- **Tool-generated outputs survive uninstall as orphans. (DIAGNOSED 2026-05-22, NOT YET FIXED.)** FNIS / Pandora / Nemesis run as Vortex tools. The FNIS pattern (representative — `extensions/fnis-integration/src/fnis.ts`): the integration creates a synthetic mod in staging (`installPath/FNIS_Data_(profile)`), runs the tool with `RedirectFiles="${modPath}"` so its outputs land inside the synthetic mod, then emits `deploy-single-mod` so the mod gets deployed like any other. Because the synthetic mod participates in the normal deploy pipeline, the **deployed links** are tracked in the manifest. But when the tool re-runs and changes the set of generated files, the previous batch of links isn't reconciled against the new batch — and when the tool integration is disabled or the synthetic mod is removed without a clean purge, the deployed `.hkx` files stay in `Data/meshes/actors/character/behaviors/` because the per-file manifest entries got dropped along with the mod state but the on-disk links weren't walked. Symptom on the user's install: 7× `FNIS_*_Behavior.hkx` in deployed `behaviors/` long after FNIS was uninstalled. **Suggested shape:** the synthetic-mod pattern needs a teardown hook that purges the deployed output regardless of whether the mod entry is still in state. The fix surface is the tool-integration extensions themselves (`extensions/fnis-integration/`, and any future Pandora/Nemesis integration) — emit an explicit purge before deleting the synthetic mod, don't rely on natural deploy diffing. Out-of-scope of the casing-fix PR — file separately.

### plugin persistence (`PluginPersistor.ts`) — Proton foreign-reset recovery vs. upstream's external-change prompt

- **Two independent "the plugin files changed under us" mechanisms now coexist in `extensions/gamebryo-plugin-management/src/util/PluginPersistor.ts::deserialize()`/`filterFileData()` and must both survive every future merge.** Ours (fork-local, `814f758b9`, landed resolving the v2.5.0 sync): when Proton/the game itself resets `plugins.txt` to a bare header with zero plugin entries, recover silently — re-serialize the in-memory state immediately if one exists (`this.mLoaded`), or synthesize an all-enabled state from `loadorder.txt`/known-deployed-plugins on initial load, flagged via `needsRestore` and written back once `mLoaded` is true. Theirs (upstream, landed in the v2.6.x cycle): a general `foreign.detected` signal plumbed through `filterFileData`'s optional third parameter, gating an interactive `promptExternalChange()` keep/revert dialog (`mOnExternalChange`, `mExternalChoicePending`, `setExternalChangeCallback`, `adoptForeign` param on `deserialize()`) for ANY foreign rewrite while Vortex already holds state.
- **The merge shape that keeps both:** adopt upstream's `filterFileData(input, plugins, foreign?): string[]` signature and its general nested-`Promise.resolve().then().then()` structure in `deserialize()` wholesale (don't keep the fork's separate `{keys, foreignApp}`-returning variant — it duplicates what `foreign.detected` already carries). Splice the fork's Proton full-reset branch in as an early check — `if (foreign.detected && keys.length === 0)` — placed _before_ the generic `initFromKeyList`/adopt-or-prompt path, so a complete reset is caught and recovered before it ever reaches `promptExternalChange()` (which would otherwise offer to "keep" a wipe with no context to distinguish it from a deliberate foreign edit). The `mLoaded`-true branch returns `this.doSerialize() ?? Promise.resolve()` directly from the outer `.then((data) => …)` callback, which is what skips the rest of the chain (readdir/load-order resolution, adopt-or-prompt) — no separate `skipUpdate` guard flag is needed as long as that early return stays structurally an early return and doesn't get flattened into a later `.then()` stage. The `needsRestore` (initial-load, no `mLoaded` state to fall back on) branch does NOT return early — it falls through into the shared tail, calls `this.adoptParsed(newPlugins)`, then writes the recovered state back via `doSerialize()` once `adoptParsed()` has set `mLoaded = true`. It needs no separate guard against upstream's prompt either: `promptExternalChange()` only fires when `this.mLoaded` is already true, and `needsRestore` is only ever set on the branch where `this.mLoaded` was false to begin with — that field isn't set anywhere in between, so the two paths can't collide.
- **v2.6.3 sync verification:** all 19 cases in `PluginPersistor.test.ts` (upstream's suite, covering the empty-file-momentarily-zero-bytes case and the interactive keep/revert flow) pass unmodified against the merged code — none of them exercise the fork's specific "non-empty file, foreign header, zero plugin lines" shape, so a regression there wouldn't be caught by the existing suite. If this mechanism breaks in a future sync, symptom is either: the interactive prompt firing on a Proton-triggered plugins.txt wipe (should have recovered silently), or plugins.txt staying empty/broken after a wipe on initial load (should have re-enabled all known plugins). Commit for the v2.6.3 merge resolution: see the commit index.
...
- **`rebase-upstream.yml`'s `GITHUB_TOKEN` cannot push a branch that touches `.github/workflows/*.yml` — including a purely upstream-added new workflow file it never edited.** The script already restores `.github/workflows/` from `master` before pushing (`git checkout master -- .github/workflows/`, per its own comment: "GITHUB_TOKEN cannot push workflow file changes"), but `git checkout <tree> -- <pathspec>` only overwrites paths that exist in `<tree>` — it does **not** delete a path that's absent from `<tree>` but present in the working copy. When an upstream tag adds a brand-new workflow file (v2.6.3 added `annotate-mixpanel-release.yml` and `linear-release.yml`, neither on `master` at merge time), that new file survives the restore step untouched, so the final push still carries a workflow-file addition and GitHub rejects it with `refusing to allow a GitHub App to create or update workflow "…" without workflows permission`. Symptom: every scheduled run past the point a workflow file changed upstream fails at the `git push` step with exit 1, even though the merge itself (including its "conflicts detected" arm) completed and committed cleanly — **no draft PR is ever opened**, because PR creation is the step after the failed push. Observed: 6 consecutive daily runs failed this way, 2026-09-16 through 2026-09-21, targeting v2.6.0 through v2.6.3 in turn (each day's run re-attempted against whatever the "latest stable tag" resolved to that day) — `gh pr list --state open` was empty for the whole window, confirming no draft ever landed for a human to pick up.
    - **This is not fixable by editing `git checkout`'s pathspec** — the fix needs either an explicit reconciliation step (`git ls-tree` diff between the working copy and `master` for `.github/workflows/`, `git rm` anything present-only-in-working-copy) or a `GITHUB_TOKEN` replacement with `workflows` permission (a PAT secret, Alex's call whether to add one — a bot with `workflows` write is a wider blast radius than the current setup accepts by default).
    - **Until fixed, expect to do this sync manually whenever upstream added or touched a workflow file**, using a personal token/session with the `workflow` OAuth scope (`gh auth status` shows it under Token scopes) to push. Manual procedure: replicate `.github/scripts/rebase-upstream.sh` steps 1-6 by hand (fetch, branch, `api merge=ours` submodule driver, merge, resolve conflicts _for real_ rather than committing them with markers — the automation deliberately leaves markers in for human triage, a manual merge must not ship that state), run the full post-merge checklist below plus `pnpm run build`/`lint`/`typecheck`/`test`/`scripts/linux-smoke.sh`, then push and open the PR (or merge directly, per Alex's call on the day).
