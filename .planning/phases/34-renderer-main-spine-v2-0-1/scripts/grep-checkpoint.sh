#!/usr/bin/env bash
#
# grep-checkpoint.sh — Phase 34 renderer + main spine (v2.0.1) re-grep harness.
#
# Phase 34 extension of the Phase 33 12-gate harness (which extended Phase 32 v8.1
# base, which extended v8.0 Phase 26 7ed691f40). Phase 34 appends one new gate per
# [D-34-04] / RESEARCH §Validation Architecture:
#   - gate 13 single-host getIPCPath (export in src/renderer/src/util/ipc.ts +
#             ≥4 importers across {ExtensionManager.ts, extensions/symlink_activator_elevate/index.ts,
#             util/elevated.ts, util/fs.ts}) — protects the D-04 IPC serialisation
#             trap memory: any second host or dropped importer at Wave E or F3
#             resolution time fails the harness immediately.
# The existing no-conflict-marker gate keeps its slot last and is renumbered to gate 14.
#
# Gates 14/15/16 from the D-34-04 candidate list are explicitly SKIPPED:
#   - gate-14-cand: already cross-platform (no new pattern needed)
#   - gate-15-cand: covered by L1 + commit-body discipline (error classes live in
#                   src/shared/src/types/errors.ts; Phase 34 handles via per-file
#                   review, not a grep gate)
#   - gate-16-cand: trivial post-Wave-H R2 DROP (the __mocks__ tree gets removed
#                   before any test that could rely on auto-mocking runs)
#
# Total: 13 gates. Inherited 11 unconditional gates (1–11), new gate-13
# (unconditional), and gate-14 (the marker gate) which is suppressed under
# --skip-conflict-check so per-file commits in waves A–H pass while resolution
# is still in flight.
#
# Phase 33 framing (preserved verbatim below as historical context):
# Adds 5 gates per [D-33-04] / RESEARCH §4 / [D-33-05]:
...
# Gate 13: single-host getIPCPath in src/renderer/src/util/ipc.ts + ≥4 importers.
# Phase 34 addition per [D-34-04] / RESEARCH §Validation Architecture. Protects
# the D-04 IPC serialisation trap memory: getIPCPath must have exactly one
# export host (src/renderer/src/util/ipc.ts) and the four canonical importers
# must all keep their import — any drop at Wave E or F3 resolution time means
# the parent-server or stringified-child-closure dual-patch got eaten.
GETIPC_HOST="src/renderer/src/util/ipc.ts"
GETIPC_IMPORTERS="src/renderer/src/ExtensionManager.ts src/renderer/src/extensions/symlink_activator_elevate/index.ts src/renderer/src/util/elevated.ts src/renderer/src/util/fs.ts"
gate13_label="single-host getIPCPath (1 export in ipc.ts + ≥4 importers in {ExtensionManager.ts, symlink_activator_elevate/index.ts, util/elevated.ts, util/fs.ts})"
g13_export=0
if [ -f "$GETIPC_HOST" ]; then
  g13_export=$(grep -c '^export function getIPCPath' "$GETIPC_HOST" 2>/dev/null || echo 0)
fi
g13_importers=$(grep -lE 'import.*getIPCPath.*from' $GETIPC_IMPORTERS 2>/dev/null | wc -l)
if [ "$g13_export" -eq 1 ] && [ "$g13_importers" -ge 4 ]; then
  pass "$gate13_label"
else
  fail "$gate13_label" "export=${g13_export} (want 1), importers=${g13_importers} (want ≥4)"
fi

# Gate 14: no conflict markers anywhere outside .planning/.
# Phase 34 keeps this gate last and renumbers from gate 12 (Phase 33). Phase 33
# scoped the marker check to mod_management/ + extensions/ (its hand-resolution
# surface). Phase 34's hand-resolution surface is the whole repo outside
# .planning/ — 117 conflict files / ~1203 regions across renderer (68), main
# (9), preload (1), shared (5), and repo-wide leaves including the 915-region
# .github/actions/fingerprints/dist/index.js artifact. Broadening the gate is
# the only way the done-gate (run without --skip-conflict-check at end of
# Phase 34) actually asserts full eradication. Skip-mode suppresses this gate
# so per-file commits during waves A–H pass cleanly while resolution is in
# flight; done-gate runs it unsuppressed to assert all 117 files clean.
gate14_label="no conflict markers anywhere outside .planning/ (Phase 34 hand-resolution surface)"
if [ "$skip_conflict_check" -eq 1 ]; then
  printf 'SKIP: %s (--skip-conflict-check)\n' "$gate14_label"
else
  marker_files=$(git grep -l '^<<<<<<< ' 2>/dev/null | grep -v '^\.planning/' || true)
  if [ -z "$marker_files" ]; then
    pass "$gate14_label"
  else
    file_count=$(printf '%s\n' "$marker_files" | wc -l)
    fail "$gate14_label" "${file_count} file(s) still contain '<<<<<<< ' — $(printf '%s' "$marker_files" | head -5 | tr '\n' ' ')$( [ "$file_count" -gt 5 ] && echo "(+$((file_count - 5)) more)" )"
  fi
fi

printf '\n'
if [ "$failures" -eq 0 ]; then
  printf 'CHECKPOINT PASSED — %d gate(s) clean\n' "$(( skip_conflict_check == 1 ? 12 : 13 ))"
else
