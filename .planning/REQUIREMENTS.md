...
- [x] **STAM-04**: `{mygames}` variable resolves to correct Wine prefix location (`compatdata/<appid>/pfx/drive_c/users/steamuser/Documents/My Games`) on Linux; Windows resolution unchanged
- [x] **STAM-05**: Top-4-title game extensions confirmed working on Linux: Skyrim SE, Fallout 4, Cyberpunk 2077, Stardew Valley (inherits STAM-04 fix for Bethesda titles)

### Packaging / Distribution (DIST)

- [x] **DIST-01**: AppImage artifact (`vortex-setup-{version}.AppImage`) produced by electron-builder on Linux CI
- [x] **DIST-02**: Debian package (`vortex_{version}_amd64.deb`) produced by electron-builder on Linux CI; `xdg-utils` and `libasound2` listed as deb dependencies
- [x] **DIST-03**: GitHub Actions parallel Linux packaging job uploads AppImage, deb, and `latest-linux.yml` to GitHub releases alongside Windows artifacts
- [x] **DIST-04**: `latest-linux.yml` auto-generated by electron-builder; in-app auto-updater gated behind `process.env.APPIMAGE` check; explicit `linux.artifactName` set in config to prevent filename drift
