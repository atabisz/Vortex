...
describe("healInvalidKeys (LAZ-788 fix)", () => {
  ...
  // The Linux level_pivot build currently rejects malformed UTF-8 while
  // constructing the DELETE sink value. Keep the upstream regression test
  // active everywhere the native extension supports that operation.
  it.skipIf(process.platform === "linux")(
    "removes a deep clobbered key; preserves valid siblings and other hives",
    async () => {
      await inject(dbPath, [
